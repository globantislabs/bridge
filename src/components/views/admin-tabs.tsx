...
export function ApiProvidersTab() {
  ...
  async function testProvider(p: ApiProvider) {
    ...
    try {
      // For existing providers, the masked key isn't usable. We re-test by
      // calling the pipeline test endpoint instead, which uses the saved
      // key server-side. Only works for openai_translate type; others
      // require re-entering the key.
      if (p.type === 'openai_translate' && p.isPrimary && p.isActive) {
        const r = await fetch('/api/admin/providers/test')
        const d = await r.json()
        if (d.ok) {
          setTestStatus((prev) => ({ ...prev, [p.id]: { state: 'ok', detail: `Pipeline OK · engine: ${d.engine} · ${d.latencyMs}ms` } }))
          toast.success(`Pipeline works! Engine: ${d.engine}`)
        } else {
          setTestStatus((prev) => ({ ...prev, [p.id]: { state: 'fail', detail: d.error } }))
          toast.error(d.error || 'Test failed')
        }
        return
      }
      // For other types, ask for the key once and test directly
      ...
      const d = await r.json()
      if (d.ok) {
        setTestStatus((prev) => ({ ...prev, [p.id]: { state: 'ok', detail: d.detail } }))
        toast.success(`Key valid — ${d.latencyMs}ms`)
      } else {
        setTestStatus((prev) => ({ ...prev, [p.id]: { state: 'fail', detail: d.error } }))
        toast.error(d.error || 'Test failed')
      }
  ...
  async function testDialogKey() {
    ...
    try {
      ...
      const d = await r.json()
      if (d.ok) {
        setDialogTest({ state: 'ok', detail: `${d.detail} · ${d.latencyMs}ms` })
        toast.success(`Key valid — ${d.latencyMs}ms`)
      } else {
        setDialogTest({ state: 'fail', detail: d.error })
        toast.error(d.error || 'Test failed')
      }
    } catch (e: any) {
      setDialogTest({ state: 'fail', detail: e.message })
    ...
    try {
      ...
      if (d.ok) {
        setPipelineTest({
          state: 'ok',
          engine: d.engine,
          detail: `"${d.sourceText}" → "${d.translatedText}" (${d.latencyMs}ms, engine: ${d.engine})`,
        })
        toast.success(`Translation works! Engine: ${d.engine}`)
      } else {
        setPipelineTest({ state: 'fail', detail: d.error || d.detail })
        toast.error(d.error || 'Translation test failed')
      }
    } catch (e: any) {
